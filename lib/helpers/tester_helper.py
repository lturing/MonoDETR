...
class Tester(object):
    def __init__(self, cfg, model, dataloader, logger, train_cfg=None, model_name='monodetr'):
        ...
        try:
            ...
            self.cls_mean_size = dataloader.dataset.cls_mean_size
        except:
            self.max_objs = 50
    ...
    def inferenceV2(self, inputs, calibs, info):
        torch.set_grad_enabled(False)
        self.model.eval()
        ...
        # load evaluation data and move data to GPU.
        inputs = inputs.to(self.device)
        P2 = torch.tensor(calibs.P2).to(self.device)
        ...
        start_time = time.time()
        ###dn
        outputs = self.model(inputs, P2, targets, img_sizes, dn_args = 0)
        ###
        end_time = time.time()
        model_infer_time += end_time - start_time
